...
class PolicyNet(keras.Model):
	...
	def __init__(self, obs_dim, act_dim, nn_dim):
		...
		# LSTM input
		self.input_1 = keras.layers.Dense(nn_dim[0], activation = 'relu', kernel_initializer = tf.keras.initializers.VarianceScaling(scale=0.33, distribution = 'uniform', seed=105))

		# LSTM
		self.LSTM_1 = keras.layers.LSTM(128, kernel_initializer = tf.keras.initializers.VarianceScaling(scale=0.33, distribution='uniform', seed=105))
	...
	@tf.function
	def call(self, obs, hist_obs, hist_act):
		x_mem = tf.concat([hist_obs, hist_act], 2)
		x_mem = self.input_1(x_mem)
		x_mem = self.LSTM_1(x_mem)
		x_cf = self.input_2(obs)
		x = tf.concat([x_mem, x_cf], 1)
		x = self.dense_3(x)
		x = self.output_1(x)
		return x
...
class QNets(keras.Model):
	...
	def __init__(self, obs_dim, act_dim, nn_dim):
		...
		# QNet 1
		#   Memory
		self.Q1_input_1 = keras.layers.Dense(nn_dim[0], activation = 'relu', kernel_initializer = tf.keras.initializers.VarianceScaling(scale=0.33, distribution = 'uniform', seed = 10192))
		self.Q1_LSTM_1 = keras.layers.LSTM(128, kernel_initializer = tf.keras.initializers.VarianceScaling(scale=0.33, distribution='uniform', seed = 12397))
		...
		# QNet 2
		#   Memory
		self.Q2_input_1 = keras.layers.Dense(nn_dim[0], activation = 'relu', kernel_initializer = tf.keras.initializers.VarianceScaling(scale=0.33, distribution = 'uniform', seed = 23342))
		self.Q2_LSTM_1 = keras.layers.LSTM(128, kernel_initializer = tf.keras.initializers.VarianceScaling(scale=0.33., distribution='uniform', seed = 20397))
		#   Current Feature Extraction
		...
		self.Q2_output = keras.layers.Dense(1, kernel_initializer=tf.random_uniform_initializer(minval=-0.003, maxval=0.003, seed = 20330))


	@tf.function
	def call(self, obs, act, hist_obs, hist_act):
		x0 = tf.concat([hist_obs, hist_act], 2)
		# First nework memory
		x1_mem = self.Q1_input_1(x0)
		x1_mem = self.Q1_LSTM_1(x1_mem)
		# Second network memory
		x2_mem = self.Q2_input_1(x0)
		x2_mem = self.Q2_LSTM_1(x2_mem)

		x0 = tf.concat([obs, act], 1)
		# First network current Feature
		x1_cf = self.Q1_input_2(x0)
		# Second netowrk current feature
		x2_cf = self.Q2_input_2(x0)

		# Combine/output first network.
		x1 = tf.concat([x1_mem, x1_cf], 1)
		x1 = self.Q1_dense_3(x1)
		x1 = self.Q1_output(x1)

		# Combine/output second network
		x2 = tf.concat([x2_mem, x2_cf], 1)
		x2 = self.Q2_dense_3(x2)
		x2 = self.Q2_output(x2)

		return x1, x2
	@tf.function
	def Q1(self, obs, act, hist_obs, hist_act):
		x0 = tf.concat([hist_obs, hist_act], 2)
		# First nework memory
		x1_mem = self.Q1_input_1(x0)
		x1_mem = self.Q1_LSTM_1(x1_mem)
		...
		return x1
	@tf.function
	def Q2(self, obs, act, hist_obs, hist_act):
		x0 = tf.concat([hist_obs, hist_act], 2)
		# Second network memory
		x2_mem = self.Q2_input_1(x0)
		x2_mem = self.Q2_LSTM_1(x2_mem)
...
class TD3(object):
	"""
	brings together policy net an q nets
	"""
	...
	def get_action(self, obs_data, hist_obs, hist_act, stochastic = True):
		"""
			gets action from the policy net.
		"""
		action = self.pi.call(obs_data.reshape(1, -1), hist_obs, hist_act).numpy()
		if stochastic:
	...
	def train_iter(self):
		...
		if self.bs <= self.replay_buffer.size:
			# Sample batch
			this_batch = self.replay_buffer.sample_batch_with_history(self.bs, self.max_hist_len)
			# Estimate next action from sample
			next_action = self.pi_targ.call(this_batch["obs2"], this_batch["hist_obs2"], this_batch["hist_act2"])
			# Add policy noise for smoothing
			...
			# Find target values
			targ_q1, targ_q2 = self.qs_targ.call(this_batch["obs2"], noisy_next_action, this_batch["hist_obs2"], this_batch["hist_act2"])
			...
			with tf.GradientTape(watch_accessed_variables=False) as tape:
				...
				model_q1, model_q2 = self.qs(this_batch["obs1"], this_batch["act"],
						this_batch["hist_obs1"], this_batch["hist_act"])
			...
			if self.total_step_count % self.d == 0:
				...
				with tf.GradientTape(watch_accessed_variables=False) as tape:
					...
					pi_loss = -tf.reduce_mean(self.qs.Q1(this_batch["obs1"],
							self.pi(this_batch["obs1"], this_batch["hist_obs1"], this_batch["hist_act"]),
							this_batch["hist_obs1"], this_batch["hist_act"]))
	...
	def save_model(self):
		"""
			saves the weights of everything
		"""
